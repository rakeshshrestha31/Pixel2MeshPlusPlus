...
def f_score(points, labels, dist1, idx1, dist2, idx2, threshold):
    len_points = points.shape[0] + 1e-6
    len_labels = labels.shape[0] + 1e-6
...
if __name__ == '__main__':
    ...
    pred_file_list = os.path.join(
        args.save_path, args.name,
        'predict',
        str(args.test_epoch), '*_predict.xyz'
    )
    ...
    for index, pred_path in tqdm.tqdm(enumerate(xyz_list_path), total=total_num):
        ...
        model_id, sample_id = pred_path.split('/')[-1].split('_')[0:2]
        ...
        sum_pred += scores
        if model_id not in object_pred:
            object_pred[model_id] = []

        object_pred[model_id].append(scores)

    for model_id, scores in object_pred.items():
        object_pred[model_id] = np.mean(
            np.stack(scores, axis=0), axis=0
        ).tolist()

    print(sum_pred)
    log_dir = os.path.join(args.save_path, args.name, 'logs_predict_coarse')
    ...
    with open(json_path, 'w') as f:
        json.dump(object_pred, f, indent=4)
    ...
    with open(log_path, 'a') as logfile:
        ...
        means = sum_pred / length
        print(length, ' '.join(map(str, means)))
        print(length, ' '.join(map(str, means)), file=logfile)
